...
- **Slouch alert flow** (audio-only — system notification currently commented out in `NotificationManager.triggerAlert`, watch haptic is the secondary channel):
  1. Grade goes fair/poor → wait `slouchDelay = 6s` → fire `playSlouch()` (G3 → C3 descending pair) + `WatchBridge.notify(.slouch)`.
  2. Then loop: every `slouchReminderInterval = 6s` while still alerted, fire `playSlouchReminder()` (two C3 boops — same low note as the second half of the initial pair, so it reads as a "you're still slouching" sustain rather than a fresh alert) + another `.slouch` watch buzz.
  3. Grade returns to good → cancel both the initial timer and the reminder loop. After `recoveryDelay = 3s` of sustained good, fire `playRecovery()` + `.recovery`.
  4. The reminder loop lives on `PostureSoundCoach.slouchReminderTask`; cancelled in `update(.good)` and in `reset()` (calibration / scene phase).
- **Tunable timing constants** (currently set for active testing — bump back up before shipping):
  - `NotificationManager.alertDelay = 10s` (post-slouch system-notification trigger; **system notification body is currently commented out** so this is dead code, kept for the reactivation path)
  - `PostureSoundCoach.slouchDelay = 6s`, `slouchReminderInterval = 6s`, `recoveryDelay = 3s`
